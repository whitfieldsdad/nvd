...
def get_attack_navigator_layer(
    ...
    attack_data_source = get_memory_source(path=DEFAULT_ATTACK_PATH, url=DEFAULT_ATTACK_URL) if attack_data_source is None else attack_data_source
    capec_data_source = get_memory_source(path=DEFAULT_CAPEC_PATH, url=DEFAULT_CAPEC_URL) if capec_data_source is None else capec_data_source
    ...
    selected_attack_technique_ids = get_attack_technique_ids_from_cwe_ids(g, cwe_ids)
...
def get_attack_technique_ids_from_cwe_ids(g: Graph, cwe_ids: Set[str]) -> Iterator[str]:
    capec_ids  = get_capec_technique_ids_from_cwe_ids(g, cwe_ids)
    attack_ids = get_attack_technique_ids_from_capec_ids(g, capec_ids)
    return attack_ids


def get_attack_technique_ids_from_capec_ids(g: Graph, capec_ids: Set[str]) -> Set[str]:
    result = set()
    for (a, b) in g.edges:
...
def get_capec_technique_ids_from_cwe_ids(g: Graph, cwe_ids: Set[str]) -> Set[str]:
    result = set()
    for (a, b) in g.edges:
...
def get_memory_source(path: Optional[str] = None, url: Optional[str] = None):
    if os.path.exists(path):
        return get_memory_source_from_file(path)
    elif url:
        return get_memory_source_from_web(url)
    else:
        raise ValueError('Either path or url must be specified')


def get_memory_source_from_file(path: str):
    src = MemorySource()
    src.load_from_file(path)
    return src


def get_memory_source_from_web(url: str):
    with tempfile.NamedTemporaryFile() as file:
        response = requests.get(url)
        file.write(response.content)
        return get_memory_source_from_file(file.name)
